...
@Composable
fun DoorSensorScreen(label : String?,navController: NavController) {
    val isOpen = remember { mutableStateOf(true) }
    val image = if (isOpen.value) {
        painterResource(id = R.drawable.opendoor)
    } else {
        painterResource(id = R.drawable.door)
    }
    Column (
        modifier = Modifier
        ...
        Spacer(modifier = Modifier.size(20.dp))
            ...
            Button(onClick = { isOpen.value = !isOpen.value },
                ...
                Text(if(isOpen.value)"Close the door" else "Open the door",
                    color = primaryText)
            }
    }
}
...
@Composable
fun DoorSensorScreenPreview() {
    val navController = rememberNavController()
    DoorSensorScreen(label = "Living Room Door", navController = navController)
}